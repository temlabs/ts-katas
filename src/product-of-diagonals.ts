export function getPathThroughDiagonals(inputArray: number[][]):number{
    ...
    while(indexVector[0]*indexVector[1] <= (n-1)*(n-1)){
        const numbersInRightDiagonal:number[] = getNumbersInRightDiagonal(indexVector,inputArray) // get numbers in right diagonal
        let rightDiagonalProduct = multiplyNumberArray(numbersInRightDiagonal); // get sum of right diagonal

        const leftIndexVector:number[] = [indexVector[0],(indexVector[1]*-1) + (n-1)]; // reflect index vector about the y axis going through middle of array
        const numbersInLeftDiagonal:number[] = getNumbersInLeftDiagonal(leftIndexVector,inputArray) // get numbers in right diagonal
...
function getNumbersInRightDiagonal(indexPair: number[], inputArray: number[][]):number[]{
...
function getNumbersInLeftDiagonal(indexPair: number[], inputArray: number[][]):number[]{
...
export function vectorAdd(rowVector:number[], scalingVector: number[][]):number[]{
